...
BEGIN
  ...
  INSERT INTO bni_members (
    ...
    left(p_line_link, 500), COALESCE(p_tags, '[]'::jsonb), v_user_id, v_email, 'self_registered', true
...
BEGIN
  ...
  RETURN COALESCE((
    SELECT jsonb_agg(row_to_json(m)::jsonb ORDER BY m.claimed, m.name)
    FROM (
