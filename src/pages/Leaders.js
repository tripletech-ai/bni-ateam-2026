...
function leaderContactGridHTML(l) {
  const cardLink = getCardLink(l.name) || l.cardLink || '';
  const buttons = [];

  if (l.lineLink || l.lineId) {
    buttons.push(`
      <button class="leader-contact-btn line"
  ...
  if (l.facebookLink) {
    buttons.push(`
      <button class="leader-contact-btn fb"
        data-action="leader-facebook" data-link="${escAttr(l.facebookLink)}">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/></svg>
        ${escHtml(t('leaders_facebook'))}
      </button>`);
  }
  if (l.phone) {
    buttons.push(`
      <button class="leader-contact-btn"
  ...
  return `<div class="leader-contact-grid">${buttons.join('')}</div>`;
}
...
function leaderCardSecondary(l) {
  ...
  const cardBtn = cardLink
    ? `<button type="button" class="director-btn-card has-link"
        data-action="leader-card" data-link="${escAttr(cardLink)}">
        ${escHtml(t('leaders_card'))}
      </button>`
    : '';
...
function directorCardHTML(p, index, { nameSuffix = '' } = {}) {
  ...
  const cardBtn = dirCard
    ? `<button class="dir-about-btn has-link"
        data-action="director-card" data-link="${escAttr(dirCard)}">
        ${escHtml(t('leaders_card'))}
      </button>`
    : '';
  const lineBtn = (p.lineLink || p.lineId)
...
export function bindLeaderEvents(container) {
  ...
  container.addEventListener('click', e => {
    ...
    } else if (action === 'leader-facebook') {
