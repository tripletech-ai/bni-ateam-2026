...
const BNI_KEYWORDS = [
  ...
  '裝修', '建商', '創辦人', '負責人', '決策者', '保健食品', '餐廳', '室內裝修',
];

function localExtractIntent(input) {
  const fromStructure = parseStructuredInput(input);
  if (!intentIsEmpty(fromStructure) && (fromStructure.iAm.length || fromStructure.iSeek.length)) {
  ...
  const iAm = [];
  const beforeSeek = input.split(/想找|想認識|想合作|尋找|我要找|需要找/)[0] || '';
